...
def fair_whole_classifier(train_loader, valid_loader, test_loader, args, logger):
    ...
    for clf in clfs_whole:
        ...
        if args.dataset == "law":
            ...
            r = test_dat["input_real"][:, :3]
        cf_eval(r, y_factual, y_counter, a, args)
        np.save(os.path.join(args.save_path, "{}_dcevae_factual".format(args.use_real)), y_factual)
        np.save(os.path.join(args.save_path, "{}_dcevae_counter".format(args.use_real)), y_counter)
